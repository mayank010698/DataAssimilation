...
def run_particle_filter(
    ...
    result = {
        ...
        "mean_rmse": torch.mean(torch.tensor(rmse_values)),
        "std_rmse": torch.std(torch.tensor(rmse_values)),
        "mean_log_likelihood": torch.mean(torch.tensor(ll_values)) if ll_values else 0.0,
        "total_log_likelihood": torch.sum(torch.tensor(ll_values)) if ll_values else 0.0,
...
def plot_trajectory_comparison(
    ...
    time_steps = torch.tensor([d["time_idx"] for d in trajectory_data], dtype=torch.float32)
    x_true     = torch.stack([d["x_true"] for d in trajectory_data])
    x_est      = torch.stack([d["x_est"] for d in trajectory_data])
    rmse_values = torch.tensor([d["rmse"] for d in trajectory_data])
    ...
    if config.use_preprocessing:
        ...
        x_true_orig = torch.stack([system.postprocess(xt) for xt in x_true])
        x_est_orig = torch.stack([system.postprocess(xe) for xe in x_est])
    ...
    obs_times = torch.tensor(obs_times)
    if len(obs_values) > 0:
        obs_values = torch.stack(obs_values)
        obs_true_x = torch.stack(obs_true_x)
    ...
    ax1.set_ylabel("Y")
    ax1.set_zlabel("Z")
    ax1.set_title(f"3D Lorenz96 Trajectory{space_label} (Trajectory {traj_idx})")
    ax1.legend()
    ...
    ax5.axhline(
        y=torch.mean(rmse_values),
        color="red",
        linestyle="--",
        alpha=0.7,
        label=f"Mean RMSE: {torch.mean(rmse_values):.4f}",
    )
...
def main():
    ...
    print("=" * 80)
    print("Bootstrap Particle Filter for Lorenz96")
    print("=" * 80)
    ...
    configurations = [
        {
            ...
            "obs_components": [0, 2],
            "observation_operator": create_projection_matrix(40, [0, 2]),
    ...
    for config_dict in configurations:
        ...
        system = Lorenz96(config)
        data_dir = f"./Lorenz96_data_{base_config_name.lower().replace(' ', '_')}_15step"
