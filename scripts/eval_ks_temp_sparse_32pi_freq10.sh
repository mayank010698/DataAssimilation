...
DATA_DIR="/data/da_outputs/datasets/ks_n2048_len1000_dt0p2500_obs0p100_freq1_comp128of128_arctan_pnoise0p100_J128_L100p53"
...
# Rectified Flow checkpoint for RF-based methods (32pi, J64, pnoise 0.1, cd0.1)
# RF_CKPT="/data/da_outputs/rf_runs_ks/ks_pnoise0p1_J64_32pi_cd0p1_20260117_192257/final_model.ckpt"
# RF_CKPT="/data/da_outputs/rf_runs_ks/ks_pnoise0p1_J64_32pi_cd0p1_20260117_192257/checkpoints/rf-epoch=010-val_loss=0.055543.ckpt"
RF_CKPT="/data/da_outputs/rf_runs_ks/ks_pnoise0p1_J128_32pi_cd0p1_20260117_192257/final_model.ckpt"
RF_LIK_STEPS=100
RF_SAMP_STEPS=100
...
CUDA_VISIBLE_DEVICES=${GPUS[0]} python eval.py \
  ...
  --run-name "${DATE_PREFIX}_ensf_32pi_128dim_freq10" \
  --device "$DEVICE" \
  --experiment-label "$EXPERIMENT_LABEL" \
  > "logs/${DATE_PREFIX}_ensf_32pi_128dim_freq10.log" 2>&1 &
...
CUDA_VISIBLE_DEVICES=${GPUS[1]} python eval.py \
  ...
  --wandb-project "$WANDB_PROJECT" \
  --run-name "${DATE_PREFIX}_enkf_32pi_128dim_freq10" \
  --device "$DEVICE" \
  --experiment-label "$EXPERIMENT_LABEL" \
  --n-particles 50 \
  --inflation 1.0 \
  > "logs/${DATE_PREFIX}_enkf_32pi_128dim_freq10.log" 2>&1 &
...
#   --n-particles 5000 \
#   --wandb-project "$WANDB_PROJECT" \
#   --run-name "${DATE_PREFIX}_fppf_32pi_freq10" \
#   --device "$DEVICE" \
#   --experiment-label "$EXPERIMENT_LABEL" \
#   > "logs/${DATE_PREFIX}_fppf_32pi_freq10.log" 2>&1 &
...
#   --n-particles 5000 \
#   --wandb-project "$WANDB_PROJECT" \
#   --run-name "${DATE_PREFIX}_bpf_32pi_freq10" \
#   --device "$DEVICE" \
#   --experiment-label "$EXPERIMENT_LABEL" \
#   > "logs/${DATE_PREFIX}_bpf_32pi_freq10.log" 2>&1 &
...
#   --ensf-eps-b "$ENSF_EPS_B" \
#   --wandb-project "$WANDB_PROJECT" \
#   --run-name "${DATE_PREFIX}_ensf_rf_32pi_128dim_freq10" \
#   --device "$DEVICE" \
#   --experiment-label "$EXPERIMENT_LABEL" \
#   --ensf-fallback-physical \
#   > "logs/${DATE_PREFIX}_ensf_rf_32pi_128dim_freq10.log" 2>&1 &

echo "Launched ENSF, ENKF, FPPF, BPF, and EnSF+RF runs."
echo "Logs in logs/${DATE_PREFIX}_*_32pi_freq10.log"
